...
class bi_LSTM_Encoder(nn.Module):
    def __init__(
        ...
        self.lstm = nn.LSTM(
            input_dim, hidden_dim, n_layers, dropout=dropout, batch_first=True, bidirectional=True
        )
...
class bi_LSTM_Decoder(nn.Module):
    def __init__(
        self,
        output_dim: int = 1,
        input_dim: int = 1,
...
class bi_LSTM_Seq2Seq(nn.Module):
    def __init__(self, t2v: stf.Time2Vec, encoder: bi_LSTM_Encoder, decoder: bi_LSTM_Decoder):
        super().__init__()
        self.t2v = t2v
...
class bi_LSTM_Forecaster(stf.Forecaster):
    def __init__(
        self,
        d_x: int = 6,
        d_yc: int = 1,
        ...
        self.encoder = bi_LSTM_Encoder(
            input_dim=time_dim + d_yc,
            hidden_dim=hidden_dim,
            n_layers=n_layers,
            dropout=dropout_p,
        )
        self.decoder = bi_LSTM_Decoder(
            output_dim=d_yt,
        ...
        self.model = bi_LSTM_Seq2Seq(self.t2v, self.encoder, self.decoder).to(self.device)
    ...
    def add_cli(self, parser):
        ...
        parser.add_argument(
            ...
            type=int,
            default=128,
            help="Hidden dimension for biLSTM network.",
        )
        parser.add_argument(
            "--n_layers",
            type=int,

...
class SpatialFormer_Forecaster(stf.Forecaster):
    def __init__(
        self,
        d_yc: int = 1,
        d_yt: int = 1,
        ...
        self.SpatialFormer = stf.SpatialFormer_model.nn.SpatialFormer(
            d_yc=d_yc,
            d_yt=d_yt,
            d_x=d_x,
        ...
        qprint(f" *** SpatialFormer (v1.5) Summary: *** ")
    ...
    def compute_loss(self, batch, time_mask=None, forward_kwargs={}):
        ...
        #classification / accuracy NOT implemented yet
        class_loss, acc = 0.0, -1.0
    ...
    def forward_model_pass(self, x_c, y_c, x_t, y_t, output_attn=False):
        ...
        forecast_output, recon_output, (logits, labels), attn = self.SpatialFormer(
            enc_x=enc_x,
            enc_y=enc_y,
            dec_x=dec_x,
            dec_y=dec_y,
